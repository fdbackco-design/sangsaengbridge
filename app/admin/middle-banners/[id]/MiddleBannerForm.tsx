...
import { createMiddleBanner, updateMiddleBanner } from '../actions'
...
interface MiddleBannerFormProps {
  middleBanner: MiddleBanner | null
}

export default function MiddleBannerForm({ middleBanner }: MiddleBannerFormProps) {
  ...
  const [formData, setFormData] = useState({
    image_url: middleBanner?.image_url || '',
    link_url: middleBanner?.link_url || '',
    is_active: middleBanner?.is_active ?? true,
    sort_order: middleBanner?.sort_order || 0,
  })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      const result = middleBanner
        ? await updateMiddleBanner(middleBanner.id, form)
        : await createMiddleBanner(form)

      if (result.success) {
        router.push('/admin/middle-banners')
        router.refresh()
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-card shadow-soft p-6">
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mb-2">권장 크기: 383 × 214px</p>
        <input
          type="file"
          accept="image/*"
          onChange={(e) => {
        ...
        />
        {formData.image_url && (
          <div className="relative w-full aspect-[383/214] mt-2 max-w-md">
            <Image
